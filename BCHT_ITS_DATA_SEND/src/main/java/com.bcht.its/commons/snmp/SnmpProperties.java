package com.bcht.its.commons.snmp;
    ...
    public static SnmpProperties loadProperties() {
        if (snmpProperties != null)
            ...
            try {
                ...
                URL url = SnmpProperties.class.getResource("/");
                System.out.println(url.getPath());
              //  InputStream is = SnmpProperties.class.getResourceAsStream("/application.properties");//snmp_conf
                String path = url.getPath();
                String filePath="";
                if(StringUtils.endsWith(path, "target/classes/")){
                    filePath = path.substring(1,path.lastIndexOf("target/classes/"));
                }
                filePath = filePath+"/lib/snmp_conf.properties";
