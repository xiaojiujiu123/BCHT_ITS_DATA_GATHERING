...
@RabbitListener(admin = "zwBchtAdmin",bindings =
        {
                @QueueBinding(
                        value = @Queue(value = "ITS_QUEUE_TFCPASS_WF_FAIL",durable = "true",autoDelete = "false",exclusive = "false"),
                        exchange = @Exchange(value = "ITS_EXCHANGE_ITS_TFCPASS",type = ExchangeTypes.TOPIC,durable = "true",autoDelete = "false"),
                        key = "BCHT.TFCPASS.VIOLATION.FAIL"

                )
        },
        containerFactory = "zwFactory"
)
public class TfcPassWfFailDataConsumer {
    protected static Logger logger = LoggerFactory.getLogger(TfcPassWfFailDataConsumer.class);
    @Autowired
    ...
    public void process(Object obj, Channel channel, @Header(AmqpHeaders.DELIVERY_TAG) long tag){
        ...
        try {
         ...
         /*   if(StringUtils.isNotBlank(passInfo.getTpurl())){
           ...
           // rabbitTemplate.convertAndSend("ITS_EXCHANGE_ITS_TFCPASS","BCHT.TFCPASS.VIOLATION",passInfo);
            String resultMsg = HessionClient.uploadViolationVehicle(passInfo);
