...
@RabbitListener(admin = "zwBchtAdmin",bindings =
        {
                @QueueBinding(
                        value = @Queue(value = "ITS_QUEUE_TFCPASS_WF",durable = "true",autoDelete = "false",exclusive = "false"),
                        exchange = @Exchange(value = "ITS_EXCHANGE_ITS_TFCPASS",type = ExchangeTypes.TOPIC,durable = "true",autoDelete = "false"),
                        key = "BCHT.TFCPASS.VIOLATION"

                )
        },
        containerFactory = "zwFactory"
)
public class TfcPassWfConsumer {
    protected static Logger logger = LoggerFactory.getLogger(TfcPassWfConsumer.class);
    @Autowired
